    ...
    >
      <VaCardContent class="flex flex-col h-full">
        ...
        <div class="flex flex-col items-center gap-4 grow">
          ...
          <p>
            <span class="text-[var(--va-secondary)]">Contact Person: </span>
            <span v-if="getUserById(project.project_owner)">{{ getUserById(project.project_owner)!.fullname }}</span>
          </p>
          <div v-if="project.did" class="text-sm">
            <span class="text-[var(--va-secondary)]">DID:</span>
            <span>{{ project.did }}</span>
          </div>
          <VaAvatarGroup class="my-4" :options="getTeamOptions(project.team)" :max="5" />
          <ProjectStatusBadge :status="project.status" />
          <div class="text-[var(--va-secondary)] mt-2">
            Expires: {{ new Date(project.expiration_date).toLocaleDateString() }}
          </div>
        </div>
        <VaDivider class="my-6" />
        <div class="flex justify-between">
          <VaButton preset="secondary" icon="mso-edit" color="secondary" disabled @click="$emit('edit', project)" />
